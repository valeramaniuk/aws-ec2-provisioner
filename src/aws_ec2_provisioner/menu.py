...
def display_select_subnet_menu(available_subnets, vpc_id):
    ...
    print("{choice: <{fill}} {name: <{fill}}\t{id: <{extended_fill}}\t{az: <{fill}}\t{ips: <{fill}}".format(
        choice="option#",
        name="name",
        id="subnet-id",
        az="AZ",
...
def display_select_vpc_menu(available_vpcs, region):
    ...
    print("{choice: <{fill}} {name: <{fill}}\t{id: <{extended_fill}}\t{default: <{fill}}".format(
            choice="option#",
            name="name",
            id="vpc-id",
            default="default",
...
def display_select_aws_profile_menu(available_profiles):
    ...
    for i, profile in enumerate(available_profiles):
        print("{option}.{profile: <{extended_fill}} ".format(option=i, profile=profile, extended_fill=str(EXTENDED_FILL)), end='')
        if (i+1) % 3 == 0:
            print("\n")
